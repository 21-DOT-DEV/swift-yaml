...
| **Status** | **IMPLEMENTED — uncommitted, under review** (2026-07-15). Scope + design via the cold-reader Q&A (five unknowns) plus a follow-up review Q&A (multi-document doc gap, Unknown 1 = B). 28 examples implemented; three-pass green with 2 recorded known issues (§7). On merge: flip the `Specs/README.md` row to Implemented and add a Phase 6 feature entry. |
...
- **On landing:** flip the `Specs/README.md` 004 row to Implemented; add a Phase 6 feature entry ("Spec-example conformance suite") and its change-log line.
...
- **Next:** on merge, flip the `Specs/README.md` 004 row to Implemented and add the Phase 6 feature entry + change-log line.
